...
interface ControlsProps {
  activeMode: "rewrite" | "summarize" | "expand";
  setActiveMode: (mode: "rewrite" | "summarize" | "expand") => void;
  toneMix: ToneMix;
...
export default function Controls({
  ...
  const [toneSelectionMode, setToneSelectionMode] = useState<'single' | 'mix'>('single');
    ...
    <div className="flex flex-col gap-6 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-sm">
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 uppercase tracking-wider">Select Mode</h3>
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">AI POWERED</span>
          </div>
          <div className="grid grid-cols-3 gap-2 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl">
            {modes.map((m) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 py-2.5 rounded-lg transition-all duration-200 ${
                  activeMode === m.id
        ...
        {activeMode === "rewrite" && (
          <div className="space-y-6 pt-2 border-t border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-2 duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 uppercase tracking-wider">Tone Selection</h3>
              ...
              <div className="flex p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl w-fit">
                <button 
                  onClick={() => setToneSelectionMode('single')}
                  className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all duration-200 ${
                    toneSelectionMode === 'single' 
                      ? 'bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-blue-400' 
                      : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                  }`}
                >
                ...
                <button 
                  onClick={() => setToneSelectionMode('mix')}
                  className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all duration-200 ${
                    toneSelectionMode === 'mix' 
                      ? 'bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-blue-400' 
                      : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                  }`}
                >
        ...
        {activeMode === "summarize" && (
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-2 duration-500">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Summarize mode will condense your text while keeping the essential points. No extra settings needed.
            </p>
          </div>
        )}
        ...
        {activeMode === "expand" && (
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-2 duration-500">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Expand mode will add more detail and context to your writing. No extra settings needed.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
        <Button 
          variant="primary" 
          onClick={onGenerate} 
          disabled={isLoading || (activeMode === "rewrite" && total === 0)} 
        ...
        >
          {isLoading ? (
            ...
            <span className="flex items-center justify-center gap-2">
              {activeMode === "rewrite" ? "Rewrite Text" : activeMode === "summarize" ? "Summarize Text" : "Expand Text"}
            </span>
          )}
        </Button>
      </div>
